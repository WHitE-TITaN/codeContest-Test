...
You are given an array a
 of distinct integers. In one operation, you may either:
...
Note that you may choose the entire array a. For each element ai, determine if there exists some sequence of operations to transform a
into [ai]; that is, make the array a consist of only one element, which is ai . Output your answer as a binary string of length n
, where the i-th character is 1 if there exists a sequence to transform a into [ai] , and 0 otherwise.
...
class Solution {
  public:
    string findBinaryString(int n, vector<int>& arr) {
      string result(n, '0');
      ...
      for(int i = 1; i < n; ++i) {
          ...
          suffixMax[n - 1 - i] = max(suffixMax[n - i], arr[n - 1 - i]);
      }
...
int main(){
  ...
  while(numberOfTestCases--) {
    ...
    Solution obj;
    cout<<obj.findBinaryString(n, arr)<<"\n";
  }
  return 0;
}